...
df_knowledge <- data %>% 
  ...
  mutate(Knowledge_Level = case_when(
    ...
    Knowledge_Score_Percent > 79 & Knowledge_Score_Percent <= 100 ~ 2 # 2 = Good 
...
df_attitude <- data %>% 
  ...
  mutate(Attitude_Level = case_when(
    Attitude_Score_Percent >= 0 & Attitude_Score_Percent <= 49 ~ 1, # 1 = Negative
    Attitude_Score_Percent > 79 & Attitude_Score_Percent <= 100 ~2 # 2 = Positive 
...
df_practices <- data %>% 
  ...
  mutate(Practice_Level = case_when(
    Practice_Score_Percent >= 0 & Practice_Score_Percent <= 79 ~ 1, # 1 = Poor 
    Practice_Score_Percent > 79 & Practice_Score_Percent <= 100 ~ 2 #2 = Good 
